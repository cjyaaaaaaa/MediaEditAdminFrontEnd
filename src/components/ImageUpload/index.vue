...
import { resolveUploadPublicUrl, resolveResourceUrl, isAbsoluteHttpUrl } from "@/utils/objectStorageUpload"
  ...
  if (val) {
    ...
    fileList.value = list.map((item: any) => {
      if (typeof item === "string") {
        const display = resolveResourceUrl(item)
        item = { name: display, url: display }
      }
      return item
    })
  } else {
...
function handleUploadSuccess(res: UploadFileResult, file: any): void {
  if (res.code === 200) {
    const publicUrl = resolveUploadPublicUrl(res)
    uploadList.value.push({ name: publicUrl, url: publicUrl })
  ...
  for (let i in list) {
    if (undefined !== list[i].url && list[i].url.indexOf("blob:") !== 0) {
      ...
      if (isAbsoluteHttpUrl(u)) {
        strs += u.trim() + separator
      } else {
        strs += u.replace(baseUrl, "") + separator
      }
    }
  }
